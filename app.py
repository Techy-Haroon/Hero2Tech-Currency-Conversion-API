...
from flask import Flask, Response, g, render_template, request, redirect, jsonify, url_for, session, send_from_directory, make_response, flash, abort, get_flashed_messages
...
from email_utils import generate_reset_email_content, generate_email_confirmation_content
...
app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(hours=int(os.getenv('PERMANENT_SESSION_LIFETIME_HOURS')))
...
class User(db.Model):
    ...
    created_at = db.Column(db.TIMESTAMP, server_default=func.now(), nullable=False) # (This time relies on server time zone e.g., UTC+2:00)
...
class SignupForm(FlaskForm):
    username = StringField('Username', validators=[DataRequired(), Length(min=3, max=20), Regexp(r'^[A-Za-z0-9_-]+$', message="Username can only contain letters, numbers, dashes, and underscores.")], render_kw={"autocomplete": "username","minlength":"3", "maxlength":"20"})
    email = StringField('Email', validators=[DataRequired(), Email(), Length(min=6, max=45)], render_kw={"type": "email", "autocomplete": "email","minlength":"6", "maxlength":"45"})
    password = PasswordField('Password', validators=[DataRequired(), Length(min=8, max=25)], render_kw={"type": "password", "autocomplete": "new-password","minlength":"8", "maxlength":"25"})
    confirm_password = PasswordField('Confirm Password', validators=[DataRequired(), Length(min=8, max=25)], render_kw={"type": "password", "autocomplete": "new-password","minlength":"8", "maxlength":"25"})
...
class LoginForm(FlaskForm):
    email = StringField('Email', validators=[DataRequired(), Email(), Length(min=6, max=45)], render_kw={"type": "email", "autocomplete": "email","minlength":"6", "maxlength":"45"})
    password = PasswordField('Password', validators=[DataRequired(), Length(min=8, max=25)], render_kw={"type": "password", "autocomplete": "current-password","minlength":"8", "maxlength":"25"})
...
class ResetPasswordForm(FlaskForm):
    password = PasswordField('New Password', validators=[DataRequired(), Length(min=8, max=25)], render_kw={"autocomplete": "new-password", "minlength":"8", "maxlength":"25"})
    confirm_password = PasswordField('Confirm Password', validators=[DataRequired(), Length(min=8, max=25)], render_kw={"autocomplete": "new-password", "minlength":"8", "maxlength":"25"})
    submit = SubmitField('Reset Password')

class UpdateProfileForm(FlaskForm):
    username = StringField('Username', validators=[DataRequired(), Length(min=3, max=20), Regexp(r'[A-Za-z0-9_-]+$', message="Username can only contain leters, numbers, dashes, and underscores.")], render_kw={"autocomplete": "username","minlength":"3", "maxlength":"20"})
    submit = SubmitField('Update Profile')
    
class FeatureRequestForm(FlaskForm):
    name = StringField('Name', validators=[DataRequired(), Length(min=3, max=20), Regexp(r'^(?!\s)[A-Za-z]+(?:\s[A-Za-z]+)*(?<!\s)$', 
                message="Name must contain only alphabets, no leading/trailing spaces, and at least one alphabet.")], render_kw={"type":"text", "autocomplete": "name", "pattern": "^(?!\s)[A-Za-z]+(?:\s[A-Za-z]+)*(?<!\s)$", "title": "Name must contain only alphabets, no leading/trailing spaces, and at least one alphabet.", "minlength":"3", "maxlength":"20"})
    email = StringField('Email', validators=[DataRequired(), Email(), Length(min=6, max=45)], render_kw={"type": "email", "autocomplete": "email","minlength":"6", "maxlength":"45"})
    message = TextAreaField('Message', validators=[DataRequired(), Length(min=10,max=200)], render_kw={"minlength":"10", "maxlength":"200"})
...
def send_confirmation_email(user):
    ...
    msg = Message('Your Hero2Tech Account Confirmation', sender='verify@hero2tech.com', recipients=[user.email])
...
def get_usage_data(user_id):
    ...
    if cached_data is None:
        ...
        total_limit, usage_count, remaining_hits, start_timestamp = fetch_usage_data_from_db(user_id)
        cached_data = {
...
def get_action_count(user_id, action_type):
    today = date.today()
    count = ApiKeyAction.query.filter_by(user_id=user_id, action_type=action_type, action_date=today).first()
    return count.action_count if count else 0
    
def update_action_count(user_id, action_type):
    today = date.today()
    action = ApiKeyAction.query.filter_by(user_id=user_id, action_type=action_type, action_date=today).first()
    if action:
        action.action_count += 1
    else:
        new_action = ApiKeyAction(user_id=user_id, action_type=action_type, action_date=today, action_count=1)
...
def too_many_requests(e):
    ...
    if usage:
        ...
        return render_template('429.html', rate_limits={endpoint: reset_time_iso}, message='Too Many Requests. Please try again later.'), 429
    return render_template('429.html', message='Too Many Requests. Please try again later.'), 429
...
def request_feature():
    ...
    return render_template('request_feature.html', form=form, hcaptcha_site_key=app.config['HCAPTCHA_SITE_KEY'])
...
def report_problem():
    ...
    return render_template('report_problem.html', form=form, hcaptcha_site_key=app.config['HCAPTCHA_SITE_KEY'])
...
def contact_us():
    ...
    return render_template('contact.html', form=form, hcaptcha_site_key=app.config['HCAPTCHA_SITE_KEY'])
    ...
    if form.validate_on_submit():
        try:
            if form.password.data == form.confirm_password.data:
                ...
                new_user = User(username=form.username.data, email=form.email.data, password=hashed_password, first_name = True, ip_signup=user_ip)
                send_confirmation_email(new_user)
                db.session.add(new_user)
          
                db.session.commit()
                db.session.close()
                flash('A confirmation email has been sent. Please check your email.', 'info')
            ...
            if f"Duplicate entry '{username}' for key 'username'" in e:
                db.session.close()
                flash(f"Username {username} is already taken. Please enter another username.", 'warning')
            elif f"Duplicate entry '{email}' for key 'email'" in e:
                db.session.close()
                flash(f"Email {email} already exists. Please enter another email.", 'warning')
            elif "The mail server could not deliver mail" in e:
                db.session.close()
                flash(f"The email address {email} is not valid. Please enter a valid email address.")
            else:
                db.session.close()
...
def complete_signup():
    ...
    if 'in_process' in session:
        ...
        if complete_time:
            ...
            if time_elapsed > timedelta(minutes=5):
                ...
                flash('Your Signup session got expired. Please signup again.', 'warning')
                return redirect(url_for('signup'))
        else:
            del session['in_process']
    ...
    if form.validate_on_submit():
        # Perform validation
        if not form.username.data or not form.password.data or not form.confirm_password.data:
        ...
        try:
            ...
            hashed_password = hash_password(form.password.data)
            user_ip = request.remote_addr
            user = User(username=form.username.data, email=email, password=hashed_password, social_signup=True, confirmed=True, first_name = True,ip_signup=user_ip)
            db.session.add(user)
            db.session.commit()
            ...
            if f"Duplicate entry '{form.username.data}' for key 'username'" in e:
                db.session.close()
                flash(f"Username {form.username.data} is already taken. Please enter another username.", 'warning')
            elif f"Duplicate entry '{session.get('email')}' for key 'email'" in e:
                db.session.close()
                flash(f"Email {session.get('email')} has just been signed up. Maybe you signed up on another tab? Try logging out and logging in again.", 'warning')
            else:
                db.session.close()
        ...
        if user:
            if check_password(user.password, form.password.data):
                if user.confirmed:
                    ...
                    db.session.close()
                    flash(f"<p>Please confirm your email before logging in.<br>Didn't receive email or got expired?</p><div style=\"display: inline-grid; justify-content: center; align-items: center;\"><a href=\"/resend_confirmation/{user.email}\" class=\"hero-btn\" style=\"padding: 15px 20px;\">Resend Confirmation</a></div>", 'warning')
            else:
                db.session.close()
...
def google_authorized():
    ...
    if not user:
        try:
            ...
            flash("Signup successful. Please enter your info to complete signup.", 'info')
            return redirect(url_for('complete_signup'))
    ...
    if form.validate_on_submit():
        ...
        if user:
            if user.confirmed != True:
                db.session.close()
                flash(f"<p>Verify account first to Reset Passord.<br>Didn't receive verification email or got expired?</p><div style=\"display: inline-grid; justify-content: center; align-items: center;\"><a href=\"/resend_confirmation/{email}\" class=\"hero-btn\" style=\"padding: 15px 20px;\">Resend Confirmation</a></div>", 'warning')
                return redirect(url_for('login'))
            ...
            msg = Message('Reset Your Password on Hero2Tech', sender='verify@hero2tech.com', recipients=[email])
...
def reset_password(token):
    if 'user_id' in session:
        ...
        return redirect(url_for('dashboard'))
    try:
        email = serializer.loads(token, salt='password-reset-salt', max_age=300)  # Token valid for 5 minutes
    except:
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        db.session.close()
        flash('Your password has been updated. You can now log in with your new password.',' info')
        return redirect(url_for('login'))
...
def delete_api_key(key_id):
    ...
    if len(user.api_keys) <=1:
        flash('You already only have 1 API key left. You cannot delete that.', 'warning')
        return redirect(url_for('api_keys'))
    else:
    ...
    if 'user_id' in session:
        ...
        if user.first_name == False:
            ...
            if elapsed_seconds >= 86400:
                ...
                db.session.close()
                flash('You cannot change your username more than once in 24 Hours.', 'warning')
                return redirect(url_for('dashboard'))
        if form.validate_on_submit():
        ...
        return render_template('update-profile.html', user=session['username'], form=form)
    else:
...
def convert_currencies_end():
    ...
    target_currencies = [currency.strip().upper() for currency in convert_currencies_str.split(',')]
    if not amount:
        amount = 1
